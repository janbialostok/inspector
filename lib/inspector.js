...
var createRegex = function (pattern) {
	...
	if (splitPattern.length <= 1) {
		...
		return new RegExp(splitPattern[1], splitPattern[2]);
	}
};

var createNegRegex = function (pattern) {
	var splitPattern = pattern.spilt('/'),
		neg;
	if (splitPattern.length <= 1) {
		neg = '^((?!' + pattern + ').)*';
		...
		neg = '^((?!' + splitPattern[1] + ').)*';
		return new RegExp(neg, splitPattern[2]);
	}
};
...
Inspector.prototype.$like = function (field, pattern, force) {
	if (pattern.constructor.name === 'RegExp') {
		if (this.query[field] && !force) {
			var error = new Error('Query is not properly formatted to add a $like property, please use force flag to overwrite');
			this.errors.push(error);
		}
		else {
...
Inspector.prototype.$nlike = function (field, pattern, force) {
	if (pattern.constructor.name === 'RegExp') {
		if (this.query[field] && !force) {
			var error = new Error('Query is not properly formatted to add a $nlike property, please use force flag to overwrite');
			this.errors.push(error);
		}
		else {
			this.query[field] = pattern;
		...
		var regex = createNegRegex(pattern);
		if (this.query[field] && !force) {
			var error = new Error('Query is not properly formatted to add a $nlike property, please use force flag to overwrite');
			this.errors.push(error);
		}
